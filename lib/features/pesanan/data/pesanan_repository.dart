  ...
  Future<List<PesananModel>> fetchPesananList(String idKafe) async {
    try {
      final List<dynamic> response = await _supabase
          .from('data_pesanan')
          .select('*, data_meja(nomor_meja), data_detail_pesanan(*, data_produk(nama_produk))')
          .eq('id_kafe', idKafe)
          .order('waktu_pesanan', ascending: false);

      return response.map((json) => PesananModel.fromJson(Map<String, dynamic>.from(json))).toList();
    } catch (e) {
